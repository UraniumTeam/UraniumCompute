...
public struct Vector4Int : IEquatable<Vector4Int>
{
    ...
    public int this[int index]
    {
        get
        {
            ...
            return Unsafe.Add(ref Unsafe.As<Vector4Int, int>(ref this), index);
        }
        set
        {
            if ((uint)index >= 4)
            ...
            Unsafe.Add(ref Unsafe.As<Vector4Int, int>(ref this), index) = value;
        }
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vector2Int operator /(Vector4Int left, int right)
    {
        var vec = left.value / Vector128.Create(right);
        return Unsafe.ReadUnaligned<Vector2Int>(ref Unsafe.As<Vector128<int>, byte>(ref vec));
    }
