...
namespace UN
{
    ...
    public:
        ...
        inline ArraySlice(T* pData, USize count)
            : m_pBegin(pData)
            , m_pEnd(pData + count)
        {
        }
        ...
        inline ArraySlice(T* pBegin, T* pEnd)
            : m_pBegin(pBegin)
            , m_pEnd(pEnd)
        {
        }
        ...
        template<USize N>
        inline explicit ArraySlice(T (&arr)[N])
            : m_pBegin(arr)
            , m_pEnd(arr + N)
        {
        }
        ...
        inline ArraySlice(StdVector& vector) // NOLINT
        ...
        template<USize N>
        inline ArraySlice(StdArray<N>& array) // NOLINT
            : m_pBegin(array.data())
